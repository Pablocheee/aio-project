...
        async function processInput() {
    ...
    try {
        ...
        if (data.reply.includes('[DATA_READY]')) {
            // Ищем URL в истории чата (первое сообщение пользователя)
            const projectUrl = chatHistory[0].content; 
            localStorage.setItem('aio_project_url', projectUrl); 

            setTimeout(() => {
                window.location.href = '/dashboard'; // Убедись, что путь совпадает с роутом в Next.js
            }, 2500);
        }
    } catch(e) { 
